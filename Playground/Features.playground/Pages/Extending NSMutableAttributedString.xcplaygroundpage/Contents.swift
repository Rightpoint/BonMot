...
//: Extend the string and specify a font, textColor and backgroundColor. This will override the font and textColor of the original style, but preserve the lineHeightMultiple
quote.extend(with: " black, ", style: BonMot(.font(UIFont(name: "SuperClarendon-Black", size: 20)!), .textColor(.white), .backgroundColor(.black)))
...
quote.extend(with: " red.", style: BonMot(.font(UIFont(name: "SuperClarendon-Black", size: 20)!), .textColor(.white), .backgroundColor(.raizlabsRed)))

quote.append(NSAttributedString(string: "\n Maria Sharapova ", attributes: BonMot(.font(UIFont(name: "SuperClarendon-Black", size: 20)!), .textColor(.raizlabsRed)).attributes()))
quote.extend(with: UIImage(named: "Tennis Racket")!, style: BonMot(.textColor(.white), .baselineOffset(-4)))
